...
经典定义来自Russell & Norvig[《人工智能：一种现代方法》]( https://github.com/thzt/book-excerpt/issues/18  "人工智能：一种现代方法")：**Agent是能感知环境并通过行动最大化预期绩效的理性实体**。在LLM时代，这一定义被升级为：**Agent = LLM + Planning + Memory + Tools + Autonomy**（自主性是灵魂）。
